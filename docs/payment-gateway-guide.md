...
BEpusdt 标签页用于对接 Upay Pro V3 加密货币收款网关，支持 USDT、USDC、TRX 等多种币种。
...
| 模式 | 接口 | 说明 |
|------|------|------|
| Upay Pro V3 兼容模式 | `/api/create_order` | 按后台配置的商户和币种创建支付订单 |
...
Upay Pro V3 兼容接口的旧版 MD5 签名只包含以下字段：
...
`merchantId` / `merchant_id` 不参与旧版 MD5 签名。

1. 取上述 5 个字段，按参数名 **ASCII 升序**排列，拼接为 `key=value&key=value`
2. 末尾追加 `appSecret`（无 `&`）
3. 对完整字符串做 **MD5**，结果转小写即为 `signature`

注意：`amount` 签名值必须与最终 JSON 请求体里的金额表示一致。例如签名时使用 `100`，发送时也发送数值 `100`。
...
回调响应要求返回字符串 `success` 或 `ok`，否则 Upay Pro V3 会按退避策略重试。

回调签名字段为 `trade_id`、`order_id`、`amount`、`actual_amount`、`token`、`block_transaction_id`、`status`，按 ASCII 升序拼接后追加 `appSecret` 并做 MD5。EdgeKey 的 `/api/payments/bepusdt/notify` 已按这个规则验签。

### 钱包配置

在 Upay Pro V3 后台「钱包」中为对应商户添加收款地址，币种格式为：

| 币种 | 说明 |
|----------|------|
| `USDT-TRC20` | USDT on Tron |
| `USDT-BSC` | USDT on BSC |
| `USDT-Polygon` | USDT on Polygon |
| `USDC-Polygon` | USDC on Polygon |
| `TRX` | TRX on Tron |

> **注意**：`paymentType` 必须和 Upay Pro V3 钱包配置中的币种完全一致。
