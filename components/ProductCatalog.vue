<template>
  <section class="grid gap-6 lg:grid-cols-[280px_1fr]">
    ...
    <div class="min-w-0">
      ...
      <div v-if="sortedProducts.length" class="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-5 xl:grid-cols-4">
        <article v-for="product in sortedProducts" :key="product.id" class="group overflow-hidden rounded-[18px] border border-base-300 bg-base-100 shadow-sm transition duration-300 hover:-translate-y-1.5 hover:scale-[1.015] hover:border-primary/30 hover:shadow-xl">
